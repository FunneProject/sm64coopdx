...
static void djui_panel_discord_destroy(UNUSED struct DjuiBase* caller) {
    // god this is so hacky and terrible
    if (sLanguageChanged) {
...
void djui_panel_discord_create(struct DjuiBase* caller) {
    struct DjuiThreePanel* panel = djui_panel_menu_create(DLANG(DISCORD, DISCORD));
